...
static void
pic_remap(int pic1_offset, int pic2_offset)
{
	uint8_t mask1, mask2;

	// when PICs are uninitialized reading from their data ports gives us
	// their active masks
	mask1 = inb(PIC1_DATA);
	mask2 = inb(PIC2_DATA);
	...
	// ICW4: have the PICs use 8086 mode (rather than the default 8080
	// mode)
...
pic_set_mask(uint8_t irq_line)
{
	...
	else
	{
		...
		irq_line -= 8;    // both PICs data register is 8 bits so
		                  // decrement irq_line by 8
	}
...
static void
pic_clear_mask(uint8_t irq_line)
{
	uint16_t port;
	uint8_t value;
...
/*
 * if the irq is >= 8 it means that the slave PIC generated it, and thus we
 * must send the PIC_EOI command to both PICs on the orher hand, if the master
 * PIC generated the interrupt, we only send PIC_EOI to it
 */
void
